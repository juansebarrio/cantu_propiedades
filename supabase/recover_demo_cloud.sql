-- ═══════════════════════════════════════════════════════════════════════════
-- RECUPERACIÓN · base demo de cloud quedó vacía por el cron roto
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Contexto: hasta el commit que arregla este bug, el cron diario
-- /api/cron/reset-seed borraba datos en JS y después llamaba a
-- seed_demo_data(). Como esa función nunca se cargó al cloud (no estaba
-- en cantu_cloud_setup.sql original), el delete corría pero el seed
-- fallaba, dejando la base vacía.
--
-- Este snippet:
--   1. Crea la tabla novedades (si todavía no existe en el cloud)
--   2. Define seed_demo_data() + reset_and_seed_demo_data()
--   3. Repuebla todo llamando a reset_and_seed_demo_data()
--
-- Cómo correrlo:
--   1. Supabase dashboard → SQL Editor → New query
--   2. Pegar este archivo entero → Run
--   3. Verificar: SELECT count(*) FROM propiedades · debería dar 7
--
-- Idempotente: se puede correr varias veces sin romper nada.
-- ═══════════════════════════════════════════════════════════════════════════


-- ── PASO 1 · tabla novedades (idempotente) ────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_novedades_creado_en_desc ON public.novedades (creado_en DESC);
CREATE INDEX IF NOT EXISTS idx_novedades_autor_id        ON public.novedades (autor_id);
...
DROP POLICY IF EXISTS "novedades_update_vista_por" ON public.novedades;
...
BEGIN
  ...
  IF v_zulma_id IS NULL OR v_martin_id IS NULL OR v_carolina_id IS NULL THEN
    RAISE EXCEPTION 'No se encontraron los 3 usuarios base (socia_titular/socio_operativo/administrativa).';
  END IF;
...
-- ── PASO 3 · función reset_and_seed_demo_data() ───────────────────
...
-- ── PASO 4 · repoblar la base AHORA ───────────────────────────────

SELECT public.reset_and_seed_demo_data();


-- ── PASO 5 · verificación · esto debería dar 6/7/5/3 ──────────────

SELECT 'duenos' AS tabla, count(*) AS total FROM public.duenos
